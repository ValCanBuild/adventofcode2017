...
let rowStrings = data.lines
let numRows = rowStrings.count

var sum = 0

for row in rowStrings {
    let stringEntries = row.components(separatedBy: "    ")
    let rowNumbers = stringEntries.map( {Int($0)!} )
    
    var division = 0
    
    for i in 0..<rowNumbers.count {
        let number = rowNumbers[i]
        
        for j in i+1..<rowNumbers.count {
            let secondNumber = rowNumbers[j]
            
            if (number % secondNumber == 0) {
                division = number / secondNumber
            } else if (secondNumber % number == 0) {
                division = secondNumber / number
            }
